...
class Logger {
  constructor() {
    this.logs = [];
    this.logFile = path.join(CONFIG.LOG_DIR, `fetch-100k-${this.getTimestamp()}.log`);
...
class ResultsSaver {
  ...
  save(results) {
    ...
    const newWordsFile = path.join(CONFIG.RESULTS_DIR, `new-words-100k-${timestamp}.json`);
    fs.writeFileSync(newWordsFile, JSON.stringify(results.validWords, null, 2));
    this.logger.success(`?? New words saved: ${newWordsFile}`);

    const deadEndFile = path.join(CONFIG.RESULTS_DIR, `dead-end-100k-${timestamp}.json`);
    ...
    const summaryFile = path.join(CONFIG.RESULTS_DIR, `summary-100k-${timestamp}.json`);
...
async function main() {
  ...
  try {
    logger.info("?? Starting 100K Words Dictionary Fetcher");
    logger.info(`??  Target: ${CONFIG.TARGET_WORDS} words (2-syllable only)`);
