...
const CONFIG = {
  HF_DATASETS: [
    "https://huggingface.co/datasets/tsdocode/vietnamese-dictionary/raw/main/data.csv",
    "https://huggingface.co/datasets/npvinHnivqn/VietnameseDictionary/raw/main/data.csv"
  ],
  TIMEOUT: 60000,
...
class Logger {
  constructor() {
    this.logs = [];
    this.logFile = path.join(CONFIG.LOG_DIR, `fetch-hf-${this.getTimestamp()}.log`);
...
class HFDatasetFetcher {
  ...
  async fetchAllDatasets() {
    this.logger.info("?? Downloading Hugging Face datasets...");
    
    for (const url of CONFIG.HF_DATASETS) {
      try {
        this.logger.info(`?? Downloading: ${url.substring(0, 60)}...`);
        const data = await this.fetchDataset(url);
        const words = this.parseCSV(data);
        this.allWords.push(...words);
        this.logger.success(`? Downloaded: ${words.length} words`);
      } catch (error) {
        this.logger.info(`??  Could not download dataset, trying next...`);
      }
    }

    const unique = [...new Set(this.allWords)];
...
class ResultsSaver {
  ...
  save(results) {
    ...
    const newWordsFile = path.join(CONFIG.RESULTS_DIR, `new-words-hf-${timestamp}.json`);
    ...
    const summaryFile = path.join(CONFIG.RESULTS_DIR, `summary-hf-${timestamp}.json`);
    fs.writeFileSync(summaryFile, JSON.stringify(summary, null, 2));
    this.logger.success(`?? Summary saved: ${summaryFile}`);

    return { newWordsFile, summaryFile };
  }
}
...
async function main() {
  ...
  try {
    logger.info("?? Starting Hugging Face Dictionary Fetcher");
    logger.info(`??  Source: Hugging Face Vietnamese Datasets`);

    const fetcher = new HFDatasetFetcher(logger);
    const allWords = await fetcher.fetchAllDatasets();
