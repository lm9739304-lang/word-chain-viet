...
const CONFIG = {
  WORDLIST_URL: "https://vietnamese-wordlist.duyet.net/Viet74K.txt",
  TIMEOUT: 60000,
...
class Logger {
  constructor() {
    this.logs = [];
    this.logFile = path.join(CONFIG.LOG_DIR, `fetch-wordlist-${this.getTimestamp()}.log`);
...
class WordlistFetcher {
  ...
  async fetchWordlist() {
    return new Promise((resolve, reject) => {
      this.logger.info(`?? Downloading Vietnamese wordlist from: ${CONFIG.WORDLIST_URL}`);
      ...
      https.get(CONFIG.WORDLIST_URL, options, (res) => {
  ...
  parseWordlist(data) {
    this.logger.info("?? Parsing wordlist...");
    const lines = data.split("\n").map(line => line.trim()).filter(line => line.length > 0);
    this.logger.success(`? Parsed ${lines.length} total words`);
    return lines;
  }
}
...
async function main() {
  ...
  try {
    logger.info("?? Starting Vietnamese Wordlist Fetcher");
    logger.info(`??  Source: Duyet Vietnamese Wordlist (Viet74K)`);

    const fetcher = new WordlistFetcher(logger);
    const wordlistData = await fetcher.fetchWordlist();
    const allWords = fetcher.parseWordlist(wordlistData);
