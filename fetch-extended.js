...
class Logger {
  constructor() {
    this.logs = [];
    this.logFile = path.join(CONFIG.LOG_DIR, `fetch-extended-${this.getTimestamp()}.log`);
...
class ResultsSaver {
  ...
  save(results) {
    ...
    const newWordsFile = path.join(CONFIG.RESULTS_DIR, `new-words-extended-${timestamp}.json`);
    fs.writeFileSync(newWordsFile, JSON.stringify(results.validWords, null, 2));
    this.logger.success(`?? New words saved: ${newWordsFile}`);

    const deadEndFile = path.join(CONFIG.RESULTS_DIR, `dead-end-extended-${timestamp}.json`);
    ...
    const summaryFile = path.join(CONFIG.RESULTS_DIR, `summary-extended-${timestamp}.json`);
...
async function main() {
  ...
  try {
    logger.info("?? Starting Extended Dictionary Fetcher");
    logger.info(`??  Source: Duyet Vietnamese Wordlist (Viet74K) - Extended Search`);
