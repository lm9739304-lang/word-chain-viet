...
class DictionaryUpdater {
  ...
  loadNewWords() {
    ...
    const resultFiles = fs.readdirSync(RESULTS_DIR).filter(f => f.startsWith("new-words-extended-"));
    
    if (resultFiles.length === 0) {
      throw new Error("No new-words-extended files found in results directory");
    }
